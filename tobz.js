...
INGAT JANGAN JUAL SCRIPT R E V KEPADA ORANG LAIN!
    ...
    try {
        ...
        // FUNCTION
                ...
                if (isGroupMsg && isDetectorBadword && !isGroupAdmins && !isAdmin && !isOwner){
                    if (chats.match("anjing") || chats.match("gblk") || chats.match("tolol") || chats.match("kntl")) {
                        if (!isGroupAdmins) {
                            ...
                            }).catch(() => tobz.sendText(from, `Untung R E V Bukan Admin, Kalo Jadi Admin Udah Aku Kick Tuh! 😑`))
                        } else {
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin R E V!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner R E V!', id)
        ...
        case '#banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner R E V!', id)
...
${desc}`)
        ...
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin R E V!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
...
🌐️ *URL*: ${url}`
		...
		case '#alkitab':
		case '#Alkitab':
            //if (args.length === 1) return tobz.reply(from, 'Kirim perintah #alkitab (ayat), contoh #alkitab yohanes 1:1', id)
            const querykitab = body.slice(9)
            console.log(querykitab)
            const alkitab = await get.get('https://api.haipbis.xyz/searchalkitab?q='+ querykitab).json()
            let Nkitab = []
            for (let i = 0; i < 25; i++) {
              Nkitab.push(i)
            }
            let kitab = `${querykitab}\n\n`
            for (let i = 0; i < Nkitab.length; i++) {
                kitab += `Ayat : ${alkitab[`${Nkitab[i]}`].ayat}\n*Isi :* ${alkitab[`${Nkitab[i]}`].isi}\n*Link :* ${alkitab[`${Nkitab[i]}`].link}\n\n`
            }
            await tobz.reply(from, kitab, id)
            limitAdd(serial)
            break
        case '#jadwalshalat':
        case '#jadwalsholat':
        ...
        case '#ytmp4':
            ...
            break
        case '#play':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin R E V!`, id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
...
➸ *Wilayah:* +${region_name}`
        ...
        case '#bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner R E V`, id)
                bctxt = body.slice(4)
                txtbc = `*「 R E V BROADCAST 」*\n\n${bctxt}`
        ...
        case '#ownergroup':
            ...
            break
        case '#otagall': // FOR OWNER & ADMIN R E V
        case '#omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner R E V', id)
        ...
        case '#okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin R E V', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '#oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin R E V', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '#oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin R E V', id)
            await tobz.sendText(from,'R E V DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '#leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '#getses':
            ...
            break
        case '#revadmin':
            let admn = `This is list of R E V Admin\nTotal : ${adminNumber.length}\n`
        ...
        case '#addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner R E V!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin R E V!', id)
                }
            break
        case '#deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner R E V!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin R E V!', id)
            break
        case '#block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner R E V!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '#ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin R E V!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin R E V!`, id)
        ...
        case '#help':
            ...
            break
        case '#revgroup':
            tobz.reply(from, `Link Group R E V : https://chat.whatsapp.com/KBt8lh74kvUJ6gunrVkco4\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#iklan':
            ...
            break
        case '#adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin R E V', id)
        ...
        case '#snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup R E V, Link Ada Di Bio atau Bisa Mengetik #R E Vgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)

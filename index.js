...
const start = async (tobz = new Client()) => {
        console.log('------------------------------------------------')
        console.log(color(figlet.textSync('R E V - B O T', { horizontalLayout: 'full' })))
        console.log('------------------------------------------------')
        console.log('[DEV] R E V')
        ...
        tobz.onAddedToGroup(async (chat) => {
            if(isWhite(chat.id)) return tobz.sendText(chat.id, 'Halo aku R E V, Ketik #help Untuk Melihat List Command Ku...')
            if(mtcState === false){
                const groups = await tobz.getAllGroups()
                // BOT group count less than
                if(groups.length > groupLimit){
                    await tobz.sendText(chat.id, 'Maaf, Batas group yang dapat R E V tampung sudah penuh').then(async () =>{
                    ...
                    if(chat.groupMetadata.participants.length < memberLimit){
                        ...
                        if(!chat.isReadOnly) tobz.sendText(chat.id, 'Halo aku R E V, Ketik #help Untuk Melihat List Command Ku...')
                    }
                }
            }else{
                await tobz.sendText(chat.id, 'R E V sedang maintenance, coba lain hari').then(async () => {
                    tobz.deleteChat(chat.id)
                    tobz.leaveGroup(chat.id)
                })
            }
